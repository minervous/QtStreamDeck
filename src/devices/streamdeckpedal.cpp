#include "streamdeckpedal.h"

using namespace streamdeck;

const DeviceInterface::Configuration & StreamDeckPedal::getConfiguration() const
{
    static Configuration conf(PID, VID,
                              5, 3);
    return conf;
}

bool StreamDeckPedal::setBrightness(int procentage)
{
    if (!m_hid.isOpen()) {
        return false;
    }
    ...
    m_send[0] = 0x03u;
    m_send[1] = 0x08u;
    m_send[2] = procentage % 101; // brightness value [0..100]
...
bool StreamDeckPedal::sendImage(int keyIndex, const QByteArray& imageData)
{
    ...
    bool success {true};
    constexpr int IMAGE_REPORT_LENGTH = 1024,
